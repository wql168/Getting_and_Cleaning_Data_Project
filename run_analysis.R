...
# The column names of training set is also read from the features.txt, and the col names are assigned into a factor, and included in the above read.table statement. 
...
colnames(compl_data) <- gsub(",","_", gsub("-", "_", gsub("\\)","", gsub("\\(", "", as.vector(read.table('./features.txt')[["V2"]])))))



# Step 2: Extracts only the measurements on the mean and standard deviation for each measurement. 

# The compl_data data set has 10299 rows and 561 columns. Get only the columns with only the measurements on the mean and standard deviation for each measurement:
...
# the avtivity name info, and the subject info into the above data set.

# Read the activity_labels into a data set and assign the column a descriptive name, the resulting activity_labels data set has 10299 rows and one column.
...
# Read the subject_ID into a data set and assign the column a descriptive name, the resulting subject_ids data set has 10299 rows and one column.
...
# Join the dataset2 and the activity_names data sets with the class labels. The resulting data set has the activity labels as well as the actitivity names.
...
# This step to remove the illegal characters such as "(", ")", "-", "," from the colnames with the gsub functions has been included in the Step 1.
...
# As the functions used in thise section requires "reshape2" package, it needs to be installed and called into library.
